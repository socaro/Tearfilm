function [im_adjust, thickness] = auto_t(image,colormap,s,t_high)
%% function to automatically determine thickness profile
%% input:
% image - image
...
for i=1:(length(image(:,1,1))-s+1)
    for j=1:(length(image(1,:,1))-s+1)
        ...
        for ii=1:3
            ...
            rgb(ii)=sum(sum(block(:,:,ii)))/length(find(block(:,:,ii)~=0));%exclude zeros from mean
        end
        if sum(rgb)<200
            update=false; %exclude boundary values
        end
        if update
            k=dsearchn(colormap((kref-50):(kref+50),:),rgb);%create interval around reference signal
